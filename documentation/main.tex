...
\def\split{1.5}    % Länge bis zur Aufteilung der Pfeile
...
(\filewidth-\fold,\fileheight)--(\filewidth-\fold,\fileheight-\fold)--
(\filewidth,\fileheight-\fold)--cycle;
\node at (\filewidth/2,\fileheight/2) {XSL--FO};
\end{tikzpicture}
};
...
\coordinate (split) at (\split,0);

\draw (xml.east) -- (split);

\draw[->] (split) |- (xslt.west);
\draw[->] (split) |- (xsl.west);
...
\node (xsd) [right=1cm of xml] {\fileblock{XSD}};
